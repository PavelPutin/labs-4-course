...
Qn = linkage(XN,'weighted','euclidean');%построение дендрограммы 
...
% Метрики расстояния перечислены здесь
% https://www.mathworks.com/help/stats/linkage.html#d117e536575
% + Добавить кластеризацию с другими метриками (до Чебышева)
Qn = linkage(XN,'weighted','squaredeuclidean');%построение дендрограммы 
figure(1); dendrogram(Qn);             %отображение дендрограммы
idx = cluster(Qn,'maxclust',M); %выполнение кластеризации на основе дендрограммы
[ercl,idxn,prM] = erclust(M,NN,idx);%оценка ошибок
disp('Индекс качества кластеризации и частость ошибок'); disp([prM,ercl]);
...
Qn = linkage(XN,'weighted','seuclidean');%построение дендрограммы 
figure(1); dendrogram(Qn);             %отображение дендрограммы
idx = cluster(Qn,'maxclust',M); %выполнение кластеризации на основе дендрограммы
[ercl,idxn,prM] = erclust(M,NN,idx);%оценка ошибок
disp('Индекс качества кластеризации и частость ошибок'); disp([prM,ercl]);
...
% + Добавить кластеризацию с другими метриками
Qn = linkage(XN,'weighted','mahalanobis');%построение дендрограммы 
figure(1); dendrogram(Qn);             %отображение дендрограммы
idx = cluster(Qn,'maxclust',M); %выполнение кластеризации на основе дендрограммы
[ercl,idxn,prM] = erclust(M,NN,idx);%оценка ошибок
disp('Индекс качества кластеризации и частость ошибок'); disp([prM,ercl]);
...
% + Добавить кластеризацию с другими метриками
Qn = linkage(XN,'weighted','cityblock');%построение дендрограммы 
figure(1); dendrogram(Qn);             %отображение дендрограммы
idx = cluster(Qn,'maxclust',M); %выполнение кластеризации на основе дендрограммы
[ercl,idxn,prM] = erclust(M,NN,idx);%оценка ошибок
disp('Индекс качества кластеризации и частость ошибок'); disp([prM,ercl]);
...
% + Добавить кластеризацию с другими метриками
Qn = linkage(XN,'weighted','minkowski');%построение дендрограммы 
figure(1); dendrogram(Qn);             %отображение дендрограммы
idx = cluster(Qn,'maxclust',M); %выполнение кластеризации на основе дендрограммы
[ercl,idxn,prM] = erclust(M,NN,idx);%оценка ошибок
disp('Индекс качества кластеризации и частость ошибок'); disp([prM,ercl]);
...
% + Добавить кластеризацию с другими метриками
Qn = linkage(XN,'weighted','chebychev');%построение дендрограммы 
figure(1); dendrogram(Qn);             %отображение дендрограммы
idx = cluster(Qn,'maxclust',M); %выполнение кластеризации на основе дендрограммы
[ercl,idxn,prM] = erclust(M,NN,idx);%оценка ошибок
disp('Индекс качества кластеризации и частость ошибок'); disp([prM,ercl]);
